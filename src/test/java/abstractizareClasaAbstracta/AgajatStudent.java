package abstractizareClasaAbstracta;

public class AgajatStudent extends Persoana{


    public AgajatStudent(String nume, String prenume, Integer varsta, Double inaltime, Character sex) {
        super(nume, prenume, varsta, inaltime, sex);
    }

    @Override
    public void mananca() {

    }

    @Override
    public void doarme() {

    }

    @Override
    public void iaPauza() {

    }

    @Override
    public void iaSalar() {

    }

    @Override
    public void mergeLaServiciu() {

    }

    @Override
    public void desfasoareActivitate() {

    }

    @Override
    public void beaCafea() {

    }

    @Override
    public void incheieActivitate() {

    }

    @Override
    public void cautaDeLucru() {

    }

    @Override
    public void depuneCV() {

    }

    @Override
    public void staAcasa() {

    }

    @Override
    public void mergeLaFacultate() {

    }

    @Override
    public void invata() {

    }

    @Override
    public void daExamene() {

    }

    @Override
    public void primesteNote() {

    }

    @Override
    public void mergeLaRestante() {

    }
}
